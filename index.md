---
layout: home
exclude: true
---
<head>
    <link rel="stylesheet" href="assets/thumbnail-style.css">
</head>

## About Me
Hello! Welcome to my little website.

## Technical & Academic Portfolio
<div class="card-grid">
    <div class="card">
        <a href="03_144p3_preview.html">
            <img src="assets/thumbnails/144p3_thumb.jpg">
            <div class="overlay">
                <p class="title" style="font-size: 11px;"><b>UCLA Economic Forecasting Final Project</b><br>R forecast analysis of NYC daily K-12 attendance data.</p>
            </div>
        </a>
    </div>
    <div class="card">
        <a href="04_104fp_preview.html">
            <img src="assets/thumbnails/104fp_thumb.jpg">
            <div class="overlay">
                <p class="title" style="font-size: 11px;"><b>UCLA Data Science for Economists Final Project</b><br>R data analysis of US school district finance panel data.</p>
            </div>
        </a>
    </div>
    <div class="card">
        <a href="05_drawer_preview.html">
            <img src="assets/thumbnails/drawer_thumb.jpg">
            <div class="overlay">
                <p class="title" style="font-size: 11px;"><b>Selinium POS User Enabling Automation</b><br>Python web automation of POS drawer opening process for past employer.</p>
            </div>
        </a>
    </div>
</div>
<br>

## Some of my Art!
*coming soon*